...
module.exports = function Index({ logger, hubFeaturedContentService }) {
  ...
  router.get('/', async (req, res, next) => {
    try {
      ...
      const options = {
        ldapOpts: {
          url: path(['app', 'locals', 'ldap', 'url'], res),
          // tlsOptions: { rejectUnauthorized: false }
        },
        adminDn: path(['app', 'locals', 'ldap', 'adminDn'], res),
        adminPassword: path(['app', 'locals', 'ldap', 'adminPassword'], res),
        userPassword: path(['query', 'pwd'], req),
        userSearchBase: path(['app', 'locals', 'ldap', 'userSearchBase'], res),
        usernameAttribute: 'cn',
        username: path(['query', 'uid'], req),
        // starttls: false
      };

      let ldap; // sAMAccountName
