...
class VideosRepository {
	...
	public function all() {
		...
		foreach ($responseVideos as $video) {
			array_push($videos, new Video(
				$video->nid[0]->value, $video->title[0]->value,
				$video->field_moj_descriptio[0]->value,
				$video->field_moj_duration[0]->value, $video->field_moj_video[0]->url,
	...
	public function find($nid) {
		...
		return new Video(
			...
			$video->video_data->tags,
			!empty($video->video_data->channel_name) ? $video->video_data->channel_name : "",
	...
	public function getRecent() {
		...
		if ($responseVideos) {
			foreach ($responseVideos as $video) {
				array_push($videos, new Video(
					$video->nid, $video->title, $video->description, $video->video_url,
					!empty($video->thumbnail) ? $video->thumbnail : "",
					!empty($video->duration) ? $video->duration : "", $video->categories,
					$video->tags, !empty($video->channel_name) ? $video->channel_name : ""
				));
			}
		}
